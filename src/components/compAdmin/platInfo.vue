...
export default {
  ...
  methods:{
    getPlat(id){
      this.axios.get(`https://videogameweb.osc-fr1.scalingo.io/plateforme/one/${id}`,{headers:{
    ...
    getGamesbyPlat(id){
      this.axios.get(`https://videogameweb.osc-fr1.scalingo.io/plateforme/game/${id}`,{headers:{
           "Access-Control-Allow-Origin": "*",
           "Authorization": `bearer ${JSON.parse(localStorage.getItem('user'))}`}})
           .then(res=>{
